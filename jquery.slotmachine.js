	...
	*/
	$.fn.slotMachine = function(settings){
		...
		var	$slot = $(this), //jQuery selector
			...
			_maxLeft, //Max marginLeft offset
		...
		*/
		function _resetPosition(){
			$container.css("margin-left", _getOffset(_active.index));
		}
		...
		*/
		function _stop( nowOrRepeations, getElementFn ){
			...
			if( nowOrRepeations===true || nowOrRepeations<=1 ){
				...
				if( rnd.index===0 ){
					$container.css("margin-left", -$( rnd.el ).width() / 2 );
				}
		...
		*/
		...
		//Set max top offset
		_maxLeft = - $container.width();
		
		//Show active element
		$container.css("margin-left", _getOffset(settings.active) );
